    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filteredItems.map((item) => (
          ...
          >
            ...
            <div className="relative h-48 bg-gradient-to-br from-silver-200 via-champagne/40 to-silver-100 flex flex-col items-center justify-center p-6 border-b border-silver-200 group-hover:scale-[1.01] transition-transform">
              <div className="w-14 h-14 rounded-full bg-white/80 border border-gold/40 flex items-center justify-center text-gold shadow-sm">
                <Sparkles className="w-7 h-7" />
              </div>
              <span className="text-[11px] font-semibold text-silver-700 uppercase tracking-widest mt-3">
                {item.categoryLabel}
              </span>
              ...
              <button
                ...
                className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center gap-2 text-white text-xs font-semibold backdrop-blur-[2px] transition-opacity"
              >
            ...
            <div className="p-6 space-y-3 flex-1 flex flex-col justify-between">
              <div className="space-y-2">
                ...
                <p className="text-xs text-silver-600 leading-relaxed line-clamp-3">
                  {item.description}
                </p>
              </div>

              <div className="pt-4 border-t border-silver-100 space-y-3">
                <div className="text-[11px] text-silver-500">
                  <span className="font-semibold text-silver-700">Palet Warna: </span>
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 sm:p-8 space-y-6 shadow-2xl border border-silver-200 relative">
            <button
              onClick={() => setSelectedItem(null)}
              className="absolute top-4 right-4 p-1.5 rounded-full hover:bg-silver-100 text-silver-500"
            >
              <X className="w-5 h-5" />
            </button>

            <div>
              <span className="text-[10px] font-semibold uppercase tracking-wider text-gold px-2 py-0.5 rounded bg-gold/10 inline-block mb-2">
                {selectedItem.categoryLabel}
              </span>
              <h3 className="font-heading text-2xl font-bold text-silver-900">
                {selectedItem.title}
              </h3>
              <span className="text-xs text-silver-500 block mt-1">
            ...
            <div className="p-3.5 bg-silver-50 rounded-xl border border-silver-200 text-xs space-y-1">
              <span className="font-semibold text-silver-800 block">Tema & Palet:</span>
              <span className="text-silver-600">{selectedItem.palette}</span>
            </div>
