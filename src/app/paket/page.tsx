  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 space-y-12">
      ...
      <div className="text-center max-w-3xl mx-auto space-y-4">
        ...
        <p className="text-sm sm:text-base text-silver-600 leading-relaxed">
      ...
      <div className="space-y-4">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs font-semibold whitespace-nowrap transition-all ${
                  isActive
                    ...
                    : 'bg-white text-silver-700 hover:bg-silver-100 border border-silver-200'
                }`}
              >
                {label}
              </button>
        ...
        <div className="relative max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 text-xs sm:text-sm bg-white border border-silver-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold focus:border-gold transition-all"
          />
          {searchQuery && (
            <button
              onClick={() => setSearchQuery('')}
              className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-silver-400 hover:text-silver-600"
            >
      ...
      {loading ? (
        <div className="py-24 text-center space-y-4">
          ...
          <p className="text-sm text-silver-500">Memuat katalog paket dari server...</p>
        </div>
      ) : filteredPackages.length === 0 ? (
        <div className="py-20 text-center bg-white rounded-2xl border border-silver-200 p-8 space-y-4">
          <Info className="w-8 h-8 text-silver-400 mx-auto" />
          <h3 className="text-lg font-bold text-silver-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredPackages.map((pkg) => (
            ...
            >
              {/* Header Box */}
              <div className="bg-gradient-to-br from-white via-silver-50 to-champagne/30 p-6 border-b border-silver-200">
                <div className="flex items-center justify-between gap-2 mb-2">
                  <span className="text-[10px] font-semibold uppercase tracking-wider text-gold px-2 py-0.5 rounded bg-white border border-gold/30">
                ...
                <h3 className="font-heading text-2xl sm:text-3xl font-bold text-silver-900">
                  {pkg.name}
                </h3>
              ...
              <div className="p-6 flex-1 space-y-3">
                ...
                <ul className="space-y-2">
                  ...
                  {pkg.features.length > 6 && (
                    <li className="text-[11px] text-gold font-medium pl-6">
              ...
              <div className="p-6 pt-0 space-y-2.5">
                ...
                <button
                  ...
                  className="w-full btn-secondary !py-2 text-xs flex items-center justify-center gap-1 text-silver-700"
                >
      ...
      {selectedModalPackage && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-silver-200 flex flex-col">
            {/* Modal Header */}
            <div className="p-6 border-b border-silver-200 flex items-start justify-between bg-silver-50 rounded-t-2xl">
              <div>
                <span className="text-[10px] font-semibold uppercase tracking-wider text-gold px-2 py-0.5 rounded bg-white border border-gold/30">
                  {selectedModalPackage.category.replace('_', ' ')}
                </span>
                <h3 className="font-heading text-2xl font-bold text-silver-900 mt-2">
                  Paket {selectedModalPackage.name}
                </h3>
              </div>
              <button
                onClick={() => setSelectedModalPackage(null)}
                className="p-1.5 rounded-full hover:bg-silver-200 text-silver-500 transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-4 flex-1">
              <p className="text-xs sm:text-sm text-silver-700 leading-relaxed">
                {selectedModalPackage.description}
              </p>

              <div>
                <h4 className="text-xs font-bold uppercase tracking-wider text-silver-900 mb-3">
                  Kelengkapan Fasilitas ({selectedModalPackage.features.length} Item):
                </h4>
                <div className="bg-silver-50 rounded-xl p-4 border border-silver-200">
                  <ul className="space-y-2.5">
                    {selectedModalPackage.features.map((feat, idx) => (
                      <li key={idx} className="flex items-start gap-2.5 text-xs text-silver-800">
                        <CheckCircle2 className="w-4 h-4 text-emerald-600 mt-0.5 shrink-0" />
                        <span>{feat}</span>
                      </li>
                    ))}
              ...
              <div className="p-3.5 rounded-xl bg-champagne/40 border border-gold/30 text-xs text-silver-800 space-y-1">
                <span className="font-semibold text-gold-dark block">Ketentuan Booking:</span>
            ...
            {/* Modal Footer */}
            <div className="p-6 border-t border-silver-200 flex gap-3 justify-end bg-white rounded-b-2xl">
              <button
                onClick={() => setSelectedModalPackage(null)}
